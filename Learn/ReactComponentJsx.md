...
```
`maValeur` : la valeur actuelle de l’état
`setMaValeur` : fonction pour mettre à jour cette valeur
Lorsque tu appelles `setMaValeur(...)`, React programme une mise à jour de l’UI avec la nouvelle valeur.

**Le “Core React Loop”**
Explication du cycle de vie implicite quand on utilise `useState` :
  * Composant se rend → l’état est lu → l’UI est générée
  * Quand on met à jour l’état via `setMaValeur`, React “re-rend” le composant pour refléter la nouvelle valeur
Il montre que chaque mise à jour d’état déclenche une ré-exécution de la fonction du composant (le “render”).

**“Render vs Painting”**
Distinction entre :
  * *Render* : étape où React calcule ce que l’UI devrait être (avec le nouveau state)
  * *Painting* : partie où le navigateur dessine visuellement les changements à l’écran
Cette distinction aide à comprendre que React ne redessine pas “à l’aveugle” : il optimise via son propre système.

**Snapshot et mise à jour asynchrone**

Quand on appelle `setMaValeur`, la mise à jour de l’état n’est pas forcément immédiate ; React peut “batcher” plusieurs mises à jour.
Il explique comment prendre un **instantané (“snapshot”)** de l’état actuel, et pourquoi cela peut être important : éviter des bugs ou des incohérences dans les valeurs.
